...
1. **AI Layout Generation (Async)**: The user submits a prompt. The API immediately pushes a **Job** into the **BullMQ** queue (backed by **Redis Docker**) and returns a `jobId` instantly—without blocking the user. An **AI Worker** running in the background pops the job, calls the **Google Gemini API** to generate the JSON layout, persists the result, and emits a completion event.
...
| **Redis (Docker)** | Runs the BullMQ job queue for AI Workers and caches duplicate prompt results |
...
| **Message Queue** | BullMQ + Redis Docker (async AI job processing) |
| **Event Bus** | Apache Kafka (inter-service communication) |
| **Authentication** | Amazon Cognito (JWT) |
| **AI Engine** | Google Gemini 2.0 Flash API |
| **Infrastructure** | AWS EC2, S3, RDS, CloudFront, ALB, Route 53 |
...
Frontend sends POST /api/v1/ai/generate
        │
        ▼
AI Service pushes Job into BullMQ (Redis Docker)
        │
        ▼
...
|---|---|---|
| **Compute (EC2 & EBS)** | Single `t3a.small` in Public Subnet (~$18–$25/month) | Single `t3a.large` in Private Subnet (~$30–$35/month) |
...
| **Database (RDS)** | ✅ Using PostgreSQL Docker (~$0) | Single-AZ PostgreSQL `db.t3.micro` (~$18–$20/month) |
| **Cache (Redis)** | ✅ Using Redis Docker (~$0) | ✅ Using Redis Docker (~$0) |
...
| **Estimated Total** | **~$19–$29 / month** | **~$121–$151 / month** |
...
- If Redis is unavailable, the system can temporarily fall back to synchronous processing to maintain availability.
